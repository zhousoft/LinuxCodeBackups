...
//信号处理函数，通过管道将信号发送给epoll监听的文件描述符，实现统一事件源
...
//进程执行函数
//父进程中m_idx值为-1，子进程中m_idx为其在子进程数组中的序号(>=0),
//根据m_idx的值判断运行子进程还是父进程代码
template<typename T>
...
void processpool<T>::run_child()
{
	...
	while(!m_stop)
	{
		...
		for(int i= 0; i < number; i++)
		{
			...
			{
				int client = 0;
				//从父子进程间的管道读取数据并保存在client中，读取成功表示有新连接到来
			...
			else if( events[i].events & EPOLLIN)//其他可读数据事件，必然是客户端发送来的请求，调用实际处理类来处理
			{
